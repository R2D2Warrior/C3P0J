...
public class Wolfram extends GenericCommand
{
	...
	public void execute()
	{
		...
		try
		{
			...
			sb.append(Colors.setBold("Result: ") + wa.getResult() + " ");
			...
			if (!wa.getOtherPods().isEmpty())
			{
				event.respondToUser(Colors.setBold("Other Results: ") + StringUtils.join(wa.getOtherPods(), ", "));
				/*event.respondToUser(Colors.setBold("Use " + event.getPrefix() + event.getCommandName() + " [other result type]") +
						" to see that result.");*/
			}
		}
		catch (WAException e)
